...
def minimize_partition(
        name, language, stats, tokenizer, seg_len,
        cross_val_split,
        stride,
        input_dir,
        output_dir, is_train
    ...
    input_path = "{}/{}.conll".format(split_input_dir, name)
    ...
    output_path = "{}/{}.t5-small.{}.{}.jsonlines".format(
        split_output_dir, name, language, seg_len)
...
def minimize_split(stats, seg_len, stride, split, input_dir, output_dir):
    language = "english"
    minimize_partition("dev", language, stats, tokenizer, seg_len, split,
                       stride,
                       input_dir, output_dir, False)
    minimize_partition("train", language, stats, tokenizer, seg_len, split,
                       stride,
                       input_dir, output_dir, True)
    minimize_partition("test", language, stats, tokenizer, seg_len, split,
                       stride,
                       input_dir, output_dir, False)
...
if __name__ == "__main__":
    ...
    for cross_val_split in range(10):
        for seg_len in [4096, 2048, 1536, 1408, 1024, 832]:
            stats = collections.defaultdict(int)
            stride = seg_len // 2
            minimize_split(stats, seg_len, stride, cross_val_split, input_dir,
                           output_dir)
